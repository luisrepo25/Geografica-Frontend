...
import { coordinatesToGeoJSON, geoJSONToCoordinates, MapCoordinate } from '../../../core/models/safe-zone.model';
...
export class EditarZonaComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  polygonCoordinates: MapCoordinate[] = [];
  ...
  ngOnInit() {
    ...
    // Obtener ID de la zona desde la ruta
  ...
  ngAfterViewInit() {
    // No inicializar el mapa aquí, esperar a que la zona se cargue
  }
  ...
  private loadZone() {
    ...
    console.log('Cargando zona con ID:', this.zoneId);
    
    this.safeZoneService.getById(this.zoneId).subscribe({
      next: (zone: SafeZone) => {
        ...
        this.form.patchValue({
          ...
          hijosIds: zone.hijos.map(h => h.id)
        });

        // Convertir GeoJSON a coordenadas para el mapa
        this.polygonCoordinates = geoJSONToCoordinates(zone.poligono);
        console.log('Coordenadas del polígono:', this.polygonCoordinates);
        
        this.loadingZone = false;
        console.log('loadingZone cambiado a false');

        // Forzar detección de cambios para que el DOM se actualice
      ...
      error: (err: any) => {
        ...
        this.error = err.error?.message || 'Error al cargar la zona segura';
        this.loadingZone = false;
      }
    });
  }
  ...
  private initMap() {
    ...
    // Configurar eventos de click para editar polígono
    ...
    // Dibujar polígono existente si ya se cargó
    if (this.polygonCoordinates.length > 0) {
      this.drawExistingPolygon();
    }
  }

  private drawExistingPolygon() {
    // Limpiar solo marcadores y polígono visual, NO las coordenadas
    ...
    this.polygonCoordinates.forEach((coord, index) => {
  ...
  onSubmit() {
    ...
    this.safeZoneService.updateSafeZone(this.zoneId, {
      ...
      next: () => {
        ...
        Swal.fire({
          title: '¡Éxito!',
          text: 'Zona segura actualizada correctamente',
          icon: 'success',
      ...
      error: (err: any) => {
        ...
        Swal.fire({
          title: 'Error',
          text: err.error?.message || 'Error al actualizar la zona segura',
          icon: 'error',
          confirmButtonText: 'Aceptar',
          confirmButtonColor: '#1E5BFF'
        });
